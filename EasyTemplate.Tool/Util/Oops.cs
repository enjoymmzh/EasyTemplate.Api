...
public class Oops
{
    ...
    {
        var ex = new FriendlyException(errorMessage, 400);
        return ex;
    }
    ...
    {
        var ex = new FriendlyException(errorCode.ToLocal(), errorCode);
        ex.StatusCode = 600;
        return ex;
    }

    /// <summary>
    /// 友好异常抛出，支持多语言，会取出Request.Headers.ContentLanguage中支持的第一个语言
    /// </summary>
    /// <param name="errorCode"></param>
    /// <param name="context"></param>
    /// <returns></returns>
    public static FriendlyException Oh(ErrorCode errorCode, HttpContext? context)
    {
        var ex = new FriendlyException(errorCode.ToLocal(context?.Request.Headers.ContentLanguage), errorCode);
        ex.StatusCode = 600;
        return ex;
    }
