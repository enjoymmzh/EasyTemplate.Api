...
public static class Sql
{
                    ...
                    case DataFilterType.UpdateByObject:
                        {
                            ...
                            else if (entityInfo.PropertyName == nameof(EntityBase.UpdateTimeId))
                            {
                                var updateUserId = ((dynamic)entityInfo.EntityValue).UpdateUserId;
                                if (updateUserId == null)
                                {
                                    var value = Global.user?.Id > 0 ? Global.user.Id : 1;
                    ...
                    case DataFilterType.InsertByObject:
                        {
                            ...
                            {
                                var createUserId = ((dynamic)entityInfo.EntityValue).create_user_id;
    ...
    public static bool FakeDelete<T>(this ISqlSugarClient db, T entity) where T : EntityBase, new()
    {
        return db.Updateable(entity).ReSetValue(x => { x.IsDelete = true; })
            ...
            .UpdateColumns(x => new { x.IsDelete, x.UpdateTime, x.UpdateTimeId }, true) //允许更新的字段-AOP拦截自动设置UpdateTime、UpdateUserId
            .ExecuteCommand() > 0;
    }
    /// <summary>
    /// 仓储假删除
